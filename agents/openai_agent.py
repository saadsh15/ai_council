...
class OpenAIAgent(BaseAgent):
    def __init__(self, agent_id: str, model: str = "gpt-4o",
        ...
        super().__init__(agent_id, model, "openai", generate_timeout=generate_timeout, evaluate_timeout=evaluate_timeout)
        self.api_key = get_api_key("openai")
        self.base_url = "https://api.openai.com/v1/chat/completions"

    async def generate(self, prompt: str, context: Optional[str] = None) -> Output:
        if not self.api_key:
            raise ValueError(f"API key for {self.provider} not found. Please set OPENAI_API_KEY.")
